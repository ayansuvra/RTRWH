...
export function RooftopDetails({ onNext, isLoading, data }: RooftopDetailsProps) {
  const [length, setLength] = useState(data.rooftop?.area ? Math.sqrt(data.rooftop.area).toString() : '');
  const [width, setWidth] = useState(data.rooftop?.area ? Math.sqrt(data.rooftop.area).toString() : '');
  const [area, setArea] = useState(data.rooftop?.area?.toString() || '');
  const [roofType, setRoofType] = useState<'flat' | 'sloped' | ''>(data.rooftop?.type || '');
  ...
  const roofTypes = [
    {
      ...
      efficiency: '95-98%',
      advantages: ['Higher collection efficiency', 'Easier installation', 'Better water quality'],
      image: '🏢'
    },
    {
      value: 'sloped',
    ...
    <Card className="shadow-xl border-0 bg-white/80 backdrop-blur">
      <CardHeader className="text-center">
        ...
        <p className="text-gray-600">
          Provide your rooftop area and type to calculate water collection potential
        </p>
      </CardHeader>
      <CardContent className="space-y-6">
              ...
              >
                <CardContent className="p-4">
                  ...
                  <div className="space-y-2">
                    ...
                    <div>
                      <p className="text-xs text-gray-600 mb-1">Key advantages:</p>
        ...
        {area && roofType && (
          ...
          >
            <h4 className="text-blue-800 mb-3">Estimated Water Collection Potential</h4>
            <div className="grid grid-cols-3 gap-4 text-center">
              <div>
                <p className="text-2xl font-medium text-blue-600">
                  {(parseFloat(area) * 0.8 * (roofType === 'flat' ? 0.95 : 0.87)).toFixed(0)}L
                </p>
                <p className="text-xs text-gray-600">Per mm of rainfall</p>
              </div>
              <div>
                <p className="text-2xl font-medium text-green-600">
                  {(parseFloat(area) * 0.8 * 800 * (roofType === 'flat' ? 0.95 : 0.87) / 1000).toFixed(0)}k L
                </p>
                <p className="text-xs text-gray-600">Annual potential*</p>
              </div>
              <div>
                <p className="text-2xl font-medium text-purple-600">
                  ₹{(parseFloat(area) * 0.8 * 800 * (roofType === 'flat' ? 0.95 : 0.87) * 0.003).toFixed(0)}
                </p>
